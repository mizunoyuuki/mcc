...
- `sizeof` 演算子（式・型の両方、コンパイル時に定数に畳み込む）
...
4. **コード生成** (`generator.c`) — スタックマシンモデルで x86-64 アセンブリ（Intel構文）を出力
...
- Types: `int`, `char`, pointers (multi-level), fixed-size arrays
...
- `sizeof` operator (both expressions and types, folded at compile time)
...
4. **Code generator** (`generator.c`) — emits x86-64 assembly (Intel syntax) using a stack-machine model
